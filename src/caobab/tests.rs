...
fn test_precompute_problem() {
    ...
    for (i, c) in courses.iter().enumerate() {
        for j in 0..c.num_max {
            ...
            assert_eq!(problem.course_map[base_column + j], i,
                       "Column {} should be mapped to course {}, as it is within {} columns after {}",
                       base_column + j, i, c.num_max, base_column);
        }
    }
    ...
    for (x, p) in participants.iter().enumerate() {
        for y in 0..n {
            ...
            assert_eq!(problem.adjacency_matrix[(x, y)],
                       match choice { Some(c) => WEIGHTS[c], None => 0 },
    ...
    for x in participants.len()..n {
        for y in 0..n {
            assert_eq!(problem.adjacency_matrix[(x, y)], 0,
